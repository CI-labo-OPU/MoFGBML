...
public class MichiganOperation implements CrossoverOperator<IntegerSolution> {
	...
	public MichiganOperation(double crossoverProbability,
		...
		this.heuristicRuleGeneration = new HeuristicRuleGeneration(knowledge, consequentFactory);
	}
	...
	public List<IntegerSolution> doCrossover(double probability, IntegerSolution parent){
		Check.that(parent.getClass() == PittsburghSolution.class, "The parent must be class-" + PittsburghSolution.class.getCanonicalName());

		// Cast IntegerSolution to PittsburghSolution
		PittsburghSolution pittsburghParent = (PittsburghSolution)parent;
		List<Pair<Integer, Integer>> bounds = ((MichiganSolution)pittsburghParent.getMichiganPopulation().get(0)).getBounds();
		int NumberOfObjectives = ((MichiganSolution)pittsburghParent.getMichiganPopulation().get(0)).getNumberOfObjectives();
		int NumberOfConstraints = ((MichiganSolution)pittsburghParent.getMichiganPopulation().get(0)).getNumberOfConstraints();
		...
		DataSet train = heuristicRuleGeneration.getConsequentFactory().getTrain();
		...
		for(int i = 0; i < erroredPatternsIdx.length; i++) {
			Pattern pattern = train.getPatternWithID(erroredPatternsIdx[i]);
			Rule generatedRule = heuristicRuleGeneration.heuristicRuleGeneration(pattern);
			MichiganSolution michiganSolution = new MichiganSolution(bounds,
																	 NumberOfObjectives,
																	 NumberOfConstraints,
																	 generatedRule);
		...
		Variation<IntegerSolution> variation = new MichiganSolutionVariation<>(
													...
													crossover, mutation,
													heuristicRuleGeneration.getKnowledge(),
													heuristicRuleGeneration.getConsequentFactory());
